...
bool8 ScrCmd_messageinstant(struct ScriptContext *ctx)
{
    ...
    AddTextPrinterParameterized(0, FONT_NORMAL_SUBPIXEL, msg, 0, 1, 0, NULL);
    return FALSE;
}
...
bool8 ScrCmd_smallnamebox(struct ScriptContext *ctx) {
    const u8 *name = (const u8 *)ScriptReadWord(ctx);

    if (name == NULL)
        name = (const u8 *)ctx->data[0];
    ShowFieldSmallName(name);
    return FALSE;
}

bool8 ScrCmd_hidesmallnamebox(struct ScriptContext *ctx) {
...
bool8 ScrCmd_multichoice2(struct ScriptContext *ctx){
    ...
    if((u32)choices < 0x1000000){ //choices is a multichoiceId
        ...
        while(count < ARRAY_COUNT(menuItems)){
		    int len = StringLength((const u8 *)choices);
		    if(!len) break;
		    menuItems[count++].text = (const u8*)choices;
		    choices += len + 1;
        }
