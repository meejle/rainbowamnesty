...
static const u8 sText_Plural_s[] = _("s");
static const u8 sText_Plural_es[] = _("es");
static const u8 sText_Plural_x[] = _("x");
static const u8 sText_Plural_ies[] = _("ies");
static const u8 sText_Plural_s_period[] = _("s.");
static const u8 sText_Plural_apostrophe_s[] = _("'s");
static const u8 sText_Plural_eeth[] = _("eeth");
static const u8 sText_Plural_ves[] = _("ves");
static const u8 sText_Plural_Keys[] = _("Keys");
static const u8 sText_Plural_Scrlls[] = _("Scrlls"); // [sic]
// In future I'll probably expand the length of item names because you'd think
// we'd have progressed beyond "IT'S PSNCUREBERRY!" by now...
// Also it bugs me that I can't change "RotomCatalog" to "RotomCatalogue"
void CopyItemNameHandlePlural(u16 itemId, u8 *dst, u32 quantity)
{
    StringCopy(dst, ItemId_GetName(itemId));
    // Don't pluralise if the player is only receiving one item, or if the item's
    // name is already plural (e.g. "Safety Goggles")
    if (quantity > 1 && (itemId != ITEM_PEWTER_CRUNCHIES) && (itemId != ITEM_MAX_MUSHROOMS) && (itemId != ITEM_BLACK_GLASSES) && (itemId != ITEM_CHOICE_SPECS) && (itemId != ITEM_LEFTOVERS) && (itemId != ITEM_WISE_GLASSES) && (itemId != ITEM_SAFETY_GOGGLES) && (itemId != ITEM_PROTECTIVE_PADS) && (itemId != ITEM_HEAVY_DUTY_BOOTS) && (itemId != ITEM_DNA_SPLICERS) && (itemId != ITEM_REINS_OF_UNITY) && (itemId != ITEM_DEVON_PARTS) && (itemId != ITEM_GO_GOGGLES) && (itemId != ITEM_GOLD_TEETH) && (itemId != ITEM_LOADED_DICE) && (itemId != ITEM_FOSSILIZED_FISH))
    {
        // Berries should be handled automatically
        // E.g. "Pecha Berry" becomes "Pecha Berries"
        ...
        else
        {
            ...
            // Append "x" to the end of these item names
            // E.g. "Old Gateau" becomes "Old Gateaux"
            else if (itemId == ITEM_OLD_GATEAU)
            {
                StringAppend(dst, sText_Plural_x);
            }
            // Replace the "y" at the end of these item names with "ies"
            // E.g. "Rare Candy" becomes "Rare Candies"
            else if ((itemId == ITEM_RARE_CANDY) | (itemId == ITEM_DYNAMAX_CANDY) | (itemId == ITEM_X_ACCURACY) | (itemId == ITEM_FIRE_MEMORY) | (itemId == ITEM_WATER_MEMORY) | (itemId == ITEM_ELECTRIC_MEMORY) | (itemId == ITEM_GRASS_MEMORY) | (itemId == ITEM_ICE_MEMORY) | (itemId == ITEM_FIGHTING_MEMORY) | (itemId == ITEM_POISON_MEMORY) | (itemId == ITEM_GROUND_MEMORY) | (itemId == ITEM_FLYING_MEMORY) | (itemId == ITEM_PSYCHIC_MEMORY) | (itemId == ITEM_BUG_MEMORY) | (itemId == ITEM_ROCK_MEMORY) | (itemId == ITEM_GHOST_MEMORY) | (itemId == ITEM_DRAGON_MEMORY) | (itemId == ITEM_DARK_MEMORY) | (itemId == ITEM_STEEL_MEMORY) | (itemId == ITEM_FAIRY_MEMORY) | (itemId == ITEM_CELL_BATTERY) | (itemId == ITEM_WEAKNESS_POLICY) | (itemId == ITEM_BLUNDER_POLICY) | (itemId == ITEM_RUBY) | (itemId == ITEM_BOOSTER_ENERGY))
            {
                StringAppend(dst, sText_Plural_ies); // UNFINISHED
            }
            // Replace the "." at the end of these items names with "s."
            // E.g. "Guard Spec." becomes "Guard Specs."
            else if (itemId == ITEM_GUARD_SPEC)
            {
                StringAppend(dst, sText_Plural_s_period); // UNFINISHED
            }
            // Add "'s" to the end of these item names
            // E.g. "Exp.Candy XS" becomes "Exp.Candy XS's"
            else if ((itemId == ITEM_EXP_CANDY_XS) | (itemId == ITEM_EXP_CANDY_S) | (itemId == ITEM_EXP_CANDY_M) | (itemId == ITEM_EXP_CANDY_L) | (itemId == ITEM_EXP_CANDY_XL) | (itemId == ITEM_CHARIZARDITE_X) | (itemId == ITEM_CHARIZARDITE_Y) | (itemId == ITEM_MEWTWONITE_X) | (itemId == ITEM_MEWTWONITE_Y) | (itemId == ITEM_NORMALIUM_Z) | (itemId == ITEM_FIRIUM_Z) | (itemId == ITEM_WATERIUM_Z) | (itemId == ITEM_ELECTRIUM_Z) | (itemId == ITEM_GRASSIUM_Z) | (itemId == ITEM_ICIUM_Z) | (itemId == ITEM_FIGHTINIUM_Z) | (itemId == ITEM_POISONIUM_Z) | (itemId == ITEM_GROUNDIUM_Z) | (itemId == ITEM_FLYINIUM_Z) | (itemId == ITEM_PSYCHIUM_Z) | (itemId == ITEM_BUGINIUM_Z) | (itemId == ITEM_ROCKIUM_Z) | (itemId == ITEM_GHOSTIUM_Z) | (itemId == ITEM_DRAGONIUM_Z) | (itemId == ITEM_DARKINIUM_Z) | (itemId == ITEM_STEELIUM_Z) | (itemId == ITEM_FAIRIUM_Z) | (itemId == ITEM_PIKANIUM_Z) | (itemId == ITEM_EEVIUM_Z) | (itemId == ITEM_SNORLIUM_Z) | (itemId == ITEM_MEWNIUM_Z) | (itemId == ITEM_DECIDIUM_Z) | (itemId == ITEM_INCINIUM_Z) | (itemId == ITEM_PRIMARIUM_Z) | (itemId == ITEM_LYCANIUM_Z) | (itemId == ITEM_MIMIKIUM_Z) | (itemId == ITEM_KOMMONIUM_Z) | (itemId == ITEM_TAPUNIUM_Z) | (itemId == ITEM_SOLGANIUM_Z) | (itemId == ITEM_LUNALIUM_Z) | (itemId == ITEM_MARSHADIUM_Z) | (itemId == ITEM_ALORAICHIUM_Z) | (itemId == ITEM_PIKASHUNIUM_Z) | (itemId == ITEM_ULTRANECROZIUM_Z) | (itemId == ITEM_DOWSING_MACHINE) | (itemId == ITEM_TEACHY_TV))
            {
                StringAppend(dst, sText_Plural_apostrophe_s);
            }
            // Replace "ooth" in these item names with "eeth"
            // E.g. "DeepSeaTooth" becomes "DeepSeaTeeth"
            else if (itemId == ITEM_DEEP_SEA_TOOTH)
            {
                StringAppend(dst, sText_Plural_eeth); // UNFINISHED
            }
            // Replace "f" at the end of these item names with "ves"
            // E.g. "Red Scarf" becomes "Red Scarves"
            else if ((itemId == ITEM_RED_SCARF) | (itemId == ITEM_BLUE_SCARF) | (itemId == ITEM_PINK_SCARF) | (itemId == ITEM_GREEN_SCARF) | (itemId == ITEM_YELLOW_SCARF) | (itemId == ITEM_SILK_SCARF) | (itemId == ITEM_CHOICE_SCARF))
            {
                StringAppend(dst, sText_Plural_ves); // UNFINISHED
            }
            // Replace "Key" with "Keys" in these item names
            // E.g. "Key to Room 1" becomes "Keys to Room 1"
            else if ((itemId == ITEM_KEY_TO_ROOM_1) | (itemId == ITEM_KEY_TO_ROOM_2) | (itemId == ITEM_KEY_TO_ROOM_4) | (itemId == ITEM_KEY_TO_ROOM_6))
            {
                StringAppend(dst, sText_Plural_Keys); // UNFINISHED
            }
            // Replace "Scrll"[sic] with "Scrlls" in these item names
            // E.g. "ScrllOfDrknss" becomes "ScrllsOfDrknss"
            else if ((itemId == ITEM_SCROLL_OF_DARKNESS))
            {
                StringAppend(dst, sText_Plural_Scrlls); // UNFINISHED
            }
            // If the item's quantity is plural and it's not one of the specific
            // items above, just stick an "s" on the end
            // E.g. "Poké Ball" becomes "Poké Balls"
            else
            {
                StringAppend(dst, sText_Plural_s);
            }
        }
    }
}
